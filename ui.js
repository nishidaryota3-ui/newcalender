...
function initUI() {
    try {
        ...
        const icoPan = typeof iconPan !== 'undefined' ? iconPan : '👆';
        const icoPaint = typeof iconPaint !== 'undefined' ? iconPaint : '🖌️';
        const icoErase = typeof iconErase !== 'undefined' ? iconErase : '🧽';
        const icoTrash = typeof iconTrash !== 'undefined' ? iconTrash : '🗑️';
        const icoPrint = typeof iconPrint !== 'undefined' ? iconPrint : '🖨️';
        const icoHome = typeof iconHome !== 'undefined' ? iconHome : '🏠';
        ...
        toolsDiv.innerHTML = `
            <button id="tool-pointer" title="移動/回転切替 (V)" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:rgba(212,175,55,0.85); border:1px solid #d4af37; color:#000; padding:0; display:flex; justify-content:center; align-items:center;">${icoPan}</button>
            <button id="tool-paint" title="塗る (B)" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:transparent; border:1px solid transparent; color:#fff; padding:0; display:flex; justify-content:center; align-items:center;">${icoPaint}</button>
            <button id="tool-erase" title="消す (E)" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:transparent; border:1px solid transparent; color:#fff; padding:0; display:flex; justify-content:center; align-items:center;">${icoErase}</button>
            <hr style="border-color:rgba(255,255,255,0.1); width:100%; margin:4px 0;">
            <button id="clearBtn" title="選択色を全消去" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:transparent; border:1px solid transparent; color:#fff; padding:0; display:flex; justify-content:center; align-items:center;">${icoTrash}</button>
            <button id="printBtn" title="印刷 (A3)" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:transparent; border:1px solid transparent; color:#38bdf8; padding:0; display:flex; justify-content:center; align-items:center;">${icoPrint}</button>
            <hr style="border-color:rgba(255,255,255,0.1); width:100%; margin:4px 0;">
            <button id="homeBtn" title="新月を真上にリセット" style="width:26px; height:26px; border-radius:4px; cursor:pointer; background:transparent; border:1px solid transparent; color:#38bdf8; padding:0; display:flex; justify-content:center; align-items:center;">${icoHome}</button>
        `;
        ...
        const setTool = (tool, mode = null) => {
            ...
            if (appContainer) {
                if (tool === 'pointer') appContainer.style.cursor = window.interactionMode === 'pan' ? 'grab' : 'ew-resize';
                else if (tool === 'paint') appContainer.style.cursor = 'crosshair';
                else if (tool === 'erase') appContainer.style.cursor = 'cell';
            }
        };
    ...
    try {
        ...
        appContainer.addEventListener('mousedown', (e) => {
            ...
            if (window.currentTool === 'pointer') {
                appContainer.style.cursor = window.interactionMode === 'pan' ? 'grabbing' : 'ew-resize';
        ...
        window.addEventListener('mouseup', () => {
            window.isInteractionActive = false;
            if (window.currentTool === 'pointer' && appContainer) {
                appContainer.style.cursor = window.interactionMode === 'pan' ? 'grab' : 'ew-resize';
            }
        ...
        console.error("initInteractions Error:", e);
    }
}
